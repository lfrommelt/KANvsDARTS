...
from src.train_vanilla import train_kan_vanilla
...
# continue after determined start has run through?
continue_after = True

# overwrite starting condition
continue_params={}

dataset_folder = "datasets"
folder = "Results_vanilla"
success_metric = nrmse
metric_norm = "std"
success_threshold = 0.2
# for debugging fast-forward training
scale_steps = 1.0#/100#1/100#1.0  # / 100  # /100
# baaaaaad
...
# original experiment used first set of hps
grid = {
    "val_size": [20],  # [20, 40],
    "c_grid": [5],#[3, 5],
    "b_k": [3],#[3, 5],here
    "structure_steps": [300],#[50, 100],  # ,200],
    "lamb_l1": [0.05],#[0.05, 0.01],  # , 0.002], here maybe
    "lamb_entropy": [0.00001,],# [0.00001, 0.000005], here maybe too, why so small?
    "loss_fn": [None],
    "a_lr": [1.0],#[1, 0.5],  # ,2], here
    "symbolification_steps": [100],#[50, 100],  # ,200],
...
#random_start=np.random.randint(100)
random_start=10

print(f"starting at {random_start}")
                            ...
                            for n_v in range(1, 4):
                                ...
                                for seed in range(3):
                                    ...
                                    for combination in range(5):
                                        ...
                                        """
                                        --------------------Results_KAN/mult_False-prior_True-lin_False/k_1-nv_1-comb_3-seed_0.json--------------------
                                        ...
                                        trial_log = {
                                            ...
                                            "seed": seed,
                                            "run_logs": run_logs,
                                            "train_loss": run_logs[-1]["train_loss"][
                                                -1
                                            ],
                                        ...
                                        #best_idx=np.argmax([log["val_loss"][-1] for log in run_logs])[0]

                                        '''best_idx=np.argmax([log["success_val"] for log in run_logs])#[0]
                                        y=run_logs[best_idx]["val_loss"]
                                        plt.plot(np.arange(len(y)), y)
                                        plt.title(str(predicted_equation))
                                        plt.show()'''
