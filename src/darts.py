...
import random
import copy
from src.utils import ContextClass, NanError, Monitor
...
from autora.theorist.darts.operations import operation_factory
from src.operations import get_operation_as_sympy
...
def gumbel_softmax_context():
    ...
    original_state = ContextClass.gumbel_softmax_enabled

    # Set the state at entry
    ContextClass.gumbel_softmax_enabled = True
    try:
        yield
    finally:
        # Restore the original state on exit
        ContextClass.gumbel_softmax_enabled = original_state


@contextmanager
def discrete_context():
    # global discrete_enabled
    original_state = ContextClass.discrete_enabled

    # Set the state at entry
    ContextClass.discrete_enabled = True
    try:
        yield
    finally:
        # Restore the original state on exit
        ContextClass.discrete_enabled = original_state
...
# copy of the original MixedOp with the following differences:
# - convenience methods: now list-like with individual ops as items
# - placing it here makes sure that we have access to the extended namespace of operations
    ...
    def forward(self, x: torch.Tensor, weights: torch.Tensor) -> float:
        ...
        if True:
            ...
            op_results = torch.stack(
                [op(x) for op in self._ops], dim=-1
            )  # (op(x) for op in self._ops)
            utils.Dump.latent_states.append(
                {
                    "inputs": x,
                    **{
                        prim: res
                        for prim, res in zip(
                            self.primitives, op_results.swapaxes(0, -1)
                        )
                    },
                }
            )
            ...
            out = (w_masked * op_results).sum(
                dim=-1
            )  # w * op(x) for w, op in zip(w_masked, self._ops))
            # print(out)
        else:
            # Hard selection: one-hot, use only the selected op
            idx = torch.argmax(weights).item()
...
class DARTS(nn.Module):
    ...
    def __init__(
        ...
        self.linear = LinearDARTSLayer(
            n_inputs=self.n_nodes, n_outputs=n_outputs, randinit=train_output_layer
        )
    ...
    def fit(
        ...
        with torch.no_grad():
            ...
            for x, y_true in test_loader:
                ...
                losses_.append(
                    loss_fn(test_preds[-1], y_true.to(device)).detach().numpy()
                )
            ...
            if coefficient_step:
                monitor.coefficients = parameters_to_vector(
                    self.coefficients
                ).unsqueeze(-1)
        test_loss = 0
                    ...
                    if coeff_opti == "lbfgs":
                        try:
                            ...
                            if coeff_discretization == "gs":
                                with gumbel_softmax_context():
                                    loss_ = coefficients_optimizer.step(
                                        closure
                                    ).detach()
                        ...
                        for x, y_true in process_chunks(train_iter, ratio[0]):
                            ...
                            train_loss_batch_wise.append(
                                train_loss.detach().clone().numpy()
                            )
                            train_loss.backward()
                            ...
                            if False:
                                monitor.coefficients = torch.cat(
                                    (
                                        monitor.coefficients,
                                        parameters_to_vector(self.coefficients)
                                        .detach()
                                        .unsqueeze(-1),
                                    ),
                                    dim=-1,
                            ...
                            if False:
                                with torch.no_grad():
                                    self.linear.weights.clamp_(
                                        min=-self.linear_clamp, max=self.linear_clamp
                                    )
                                    self.linear.biases.clamp_(
                                        min=-self.linear_clamp, max=self.linear_clamp
                                    )
        ...
        if coefficient_step:
            ...
            if not np.isnan(train_losses).any():
                ...
                print(
                    f"Loss before finetuning ({backup_loss})\n      after finetuning ({train_losses[-1]})"
                )
    ...
    def to_sympy(self, variable_prefix):
        variables = sympy.symbols(
            [variable_prefix + str(i + 1) for i in range(self.n_vars)]
        )
        result = variables
        hidden_states = []
...
class LinearDARTSLayer(nn.Module):
    ...
    def to_sympy(self, inputs):
        layer_output = [
            sum([self.weights[j, i] * inputs[i] for i in range(self.n_inputs)])
            + self.biases[j]
...
class DARTSNode(nn.Module):
    def __init__(
        ...
        # No, we need empty lists for ops without params as placehoders for indexing, no again, to_sympy retrieves params by hand
    ...
    def forward(self, x):
        ...
        for i in range(self.n_inputs):
            ...
            if ContextClass.discrete_enabled:
                operation_outputs[:, i] = op(
                    x[:, i].unsqueeze(-1),
                    F.one_hot(torch.argmax(self.alphas[i, :]), num_classes=self.n_ops),
                ).squeeze()
            elif ContextClass.gumbel_softmax_enabled:
    ...
    def to_sympy(self, inputs):
        ...
        # iterate over inputs
        for i in range(self.n_inputs):
            mixed_op = self.mixed_ops[
                i
            ]  # =  self.mixed_ops[i*self.n_outputs:(i+1)*self.n_outputs]#tuple(islice(operation_iterator, self.n_outputs))
